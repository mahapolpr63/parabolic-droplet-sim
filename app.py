...
st.caption("โปรแกรมจำลองวิถีและทำนายการกระจายตัวของหยดน้ำแบบ 3 มิติเชิงทฤษฎี (RK4 & Fluid Dynamics)")
st.markdown("---")
...
# 3.2 On-Disk Spiral Trajectory Calculation (Rotating Reference Frame - Coriolis Effect)
dish_r_pts = np.linspace(0.001, r_detach, 100)
dish_theta_rel = []
current_theta = 0.0
...
for idx in range(len(dish_r_pts)-1):
    ...
    dt_step = dr / (u_s_c / math.sqrt(1.0 + 4.0 * (a**2) * (r_c**2)))
    # Coriolis relative rotation rate approximation
    dtheta = -(2.0 * Omega * (u_s_c * 2.0 * a * r_c / math.sqrt(1.0 + 4.0 * (a**2) * (r_c**2))) / (r_c + 1e-4)) * dt_step
    current_theta += dtheta
    dish_theta_rel.append(current_theta)
dish_theta_rel.append(current_theta)
dish_theta_rel = np.array(dish_theta_rel)

# 3.3 Single Stream Airborne Integration Function (RK4)
...
col3.metric("ระยะตกกระทบพื้น (R_splash)", f"{R_splash_cm:.2f} cm")
...
# ----------------------------------------------------
st.subheader("🌐 แบบจำลอง 3 มิติ: การกระจายตัว 8 ทิศทาง และเส้นโค้งบนผิวจาน (Interactive 3D)")

fig = go.Figure()

# 5.1 Paraboloid Surface Mesh
r_mesh = np.linspace(0, R_max, 40)
...
# 5.2 Generate 8 Streams (Angles 0, 45, 90, ..., 315 deg)
num_streams = 8
phi_angles = np.linspace(0, 2 * np.pi, num_streams, endpoint=False)
impact_x, impact_y = [], []

for idx, phi in enumerate(phi_angles):
    # A. On-Disk Spiral Path (Coriolis Deflection on Surface)
    disk_x = dish_r_pts * np.cos(phi + dish_theta_rel)
    disk_y = dish_r_pts * np.sin(phi + dish_theta_rel)
    disk_z = H - a * (dish_r_pts**2)
    
    fig.add_trace(go.Scatter3d(
        x=disk_x, y=disk_y, z=disk_z,
        mode='lines', line=dict(color='darkblue', width=4),
        name='เส้นทางการไหลบนผิวจาน (On-Disk Spiral)' if idx == 0 else "",
        showlegend=(idx == 0),
        hoverinfo='skip'
    ))

    # B. Airborne Trajectory with Instantaneous Velocity Hovering
    px, py, pz, pv, pt, R_sp = simulate_airborne_stream(phi)
    impact_x.append(px[-1])
    impact_y.append(py[-1])
    
    customdata = np.stack((pv, pt), axis=-1)
    ...
    fig.add_trace(go.Scatter3d(
        ...
        name='วิถีหยดน้ำในอากาศ (Airborne Trajectory)' if idx == 0 else "",
        showlegend=(idx == 0)
    ))

# 5.3 Ground Landing Ring & Impact Points (Z = 0)
ring_theta = np.linspace(0, 2 * np.pi, 100)
ring_x = R_splash_m * np.cos(ring_theta)
ring_y = R_splash_m * np.sin(ring_theta)

fig.add_trace(go.Scatter3d(
    x=ring_x, y=ring_y, z=np.zeros_like(ring_x),
    mode='lines', line=dict(color='gold', width=4, dash='dash'),
    name='วงแหวนรัศมีตกกระทบพื้น (Splash Ring)',
    hoverinfo='skip'
))

fig.add_trace(go.Scatter3d(
    x=impact_x, y=impact_y, z=np.zeros(num_streams),
    mode='markers', marker=dict(size=6, color='gold', symbol='diamond'),
    name='จุดตกกระทบพื้น (Impact Points)',
    hoverinfo='skip'
))

# 5.4 Layout Scaling & 3D Aspect Ratio
fig.update_layout(
    scene=dict(
        xaxis_title='X (m)',
        yaxis_title='Y (m)',
        zaxis_title='ความสูง Z (m)',
        aspectmode='manual',
        aspectratio=dict(x=1, y=1, z=0.65), # Maintains true 3D parabolic curvature
...
with summary_col1:
    ...
    st.write(f"• รัศมีจุดสลัดหลุด ($r_{{detach}}$): `{r_detach:.4f} m` ({r_detach*100:.2f} cm)")
    st.write(f"• ความสูงจุดสลัดหลุด ($z_0$): `{z0:.4f} m` ({z0*100:.2f} cm)")
...
with summary_col2:
    ...
    st.write(f"• ระยะตกกระทบพื้นทางทฤษฎี ($R_{{splash}}$): `{R_splash_cm:.2f} cm` ({R_splash_m:.3f} m)")
